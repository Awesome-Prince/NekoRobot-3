...
def handler(event):
    ...
    if not event.reply_to_msg_id:
         event.reply("Reply to an image or a sticker to memeify it Nigga!!")
        return
    reply_message =  event.get_reply_message()
    if not reply_message.media:
         event.reply("Provide some Text please")
        return
    file =  bot.download_media(reply_message)
    msg =  event.reply("Memifying this image! Please wait")

    if "Kittu" not in Credit:
         event.reply("this nigga removed credit line from code")
    text = str(event.pattern_match.group(1)).strip()

    if len(text) < 1:
        return  msg.reply("You might want to try `/mmf text`")
    meme =  drawText(file, text)
     bot.send_file(event.chat_id, file=meme, force_document=False)
     msg.delete()
    os.remove(meme)
